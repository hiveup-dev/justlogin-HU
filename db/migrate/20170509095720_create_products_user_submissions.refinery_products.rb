...
class CreateProductsUserSubmissions < ActiveRecord::Migration
  ...
  def up
    ...
    create_table :refinery_products_plans_user_submissions do |t|
      t.integer :plan_id
      t.integer :user_submission_id
    end

    add_index(:refinery_products_plans_user_submissions, [:plan_id, :user_submission_id], { name: 'products_plan_id_user_submission_id' })
